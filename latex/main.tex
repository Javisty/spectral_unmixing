...
$$B = E(\Theta) A + \eta$$
where $\eta$ is a white noise and $E(\Theta) A = \left( \sum_{p=1}^P E(\Theta)_{i,j,p,s} a_{i,j,p} \right)_{i,j,s}$ is the sum of the signature spectra weighted by abundances, for each pixel and spectral channel.
